...
export function CardExperience({ experience }: CardExperienceProps) {
  ...
  return (
    <div className="bg-card w-[450px] h-[400px] flex justify-center relative rounded-[20px] overflow-hidden">
      ...
      />
      <div className="flex flex-col absolute justify-between bg-card h-[50%] w-full top-[200px] px-[15px] py-[15px] gap-[5px] z-[50]">
        <div className="flex flex-row gap-[10px] w-full flex-1">
          <div className="flex flex-col flex-1">
            <div className="flex flex-row items-center justify-start gap-[20px]">
              <h3 className="font-bold leading-none flex p-0 m-0 text-[16px] text-main-dark-green">
                {experience.name}
                <span className="capitalize h-fit flex w-fit leading-none items-center text-[10px] ml-[10px] text-on-banner-text bg-banner py-[4px] px-[12px] rounded-[30px]">
                  {category.charAt(0).toUpperCase() +
                    category.slice(1).toLowerCase()}
                </span>
              </h3>
            </div>
            <p className="mt-[4px] text-[14px] p-0 m-0 h-[100px] overflow-y-auto text-dark-gray scrollbar-hide font-semibold">
          ...
          <div className="flex w-[30%] flex-col gap-[15px]">
            {labels.map(({ icon: Icon, text }, idx) => (
              <div
                key={idx}
                className="flex flex-row bg-card-labels rounded-[30px] items-center gap-2"
              >
