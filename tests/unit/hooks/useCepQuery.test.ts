...
describe("useCepQuery", () => {
  ...
  it("accepts formatted cep and calls api with digits only", async () => {
    ...
    const spy = ((cepApi.fetchAddressByZip as unknown) as Mock).mockResolvedValue(address);
  ...
  it("honors the options.enabled flag and disables query when false", () => {
    ...
    const { result } = renderHookWithProviders(() => useCepQuery("12345678", { enabled: false }));
