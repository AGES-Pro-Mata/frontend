...
describe('filter store', () => {
  ...
  it('setFilter with setQueryOnChange=true updates filters and query', () => {
    useFilterStore.getState().setFilter({ key: 'k', filterKey: 'a', value: '1', setQueryOnChange: true })
  ...
  it('setFilter with setQueryOnChange=false updates values only', () => {
    useFilterStore.getState().setFilter({ key: 'k', filterKey: 'a', value: '1', setQueryOnChange: false })
  ...
  it('initFilterState does not override existing state', () => {
    useFilterStore.getState().initFilterState({ key: 'k', initialFilters: { a: '1' }, setQueryOnChange: true })
    // mutate state
    useFilterStore.getState().setFilter({ key: 'k', filterKey: 'b', value: '2', setQueryOnChange: true })

    // second init should be a no-op
    useFilterStore.getState().initFilterState({ key: 'k', initialFilters: { a: 'x' }, setQueryOnChange: true })
  ...
  const missing = useFilterStore.getState().getFilterState({ key: 'missing', initialFilters: { a: '1' }, setQueryOnChange: false })
  ...
  const present = useFilterStore.getState().getFilterState({ key: 'another', initialFilters: { a: '1' }, setQueryOnChange: true })
  ...
  it('ignores null values when creating search params via setFilters', () => {
    useFilterStore.getState().setFilters({ key: 'k', newFilters: { a: '1', b: null as unknown as string }, setQueryOnChange: true })
  const state = useFilterStore.getState().filterStates['k']
