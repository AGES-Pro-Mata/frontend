...
vi.mock("@/hooks/useCurrentUser", () => ({ useCurrentUserProfile: () => ({ data: { id: "u1" } }) }));

describe("useExperienceAdjustments", () => {
  afterEach(() => vi.restoreAllMocks());

  it("returns adjustments filtered by current user id", async () => {
    const data = [{ id: "a1", userId: "u1" }, { id: "a2", userId: "other" }];

    (axios.get as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({ data });

    const { result } = renderHookWithProviders(() => useExperienceAdjustments());

    await waitFor(() => expect(result.current.data).toEqual([data[0]]));
  });

  it("handles API shape with data.data and nested user object", async () => {
    const payload = { data: { data: [{ id: "a1", user: { id: "u1" } }, { id: "a2", userId: "x" }] } };

    (axios.get as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(payload);

    const { result } = renderHookWithProviders(() => useExperienceAdjustments());

    await waitFor(() => expect(result.current.data).toEqual([payload.data.data[0]]));
  });
  ...
  vi.spyOn(userModule, "useCurrentUserProfile").mockImplementation(() => ({ data: undefined } as unknown as UseCurrentUserProfileReturn));
  ...
  it("propagates error when axios.get rejects", async () => {
    (axios.get as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("boom"));
