...
describe("useLogin", () => {
  ...
  it("handles first-access: shows warning and navigates to redefine", async () => {
    const resp = { statusCode: 200, data: { isFirstAccess: true, token: "tok1" } };
  ...
  expect(navigateMock).toHaveBeenCalledWith({ to: `/auth/redefine/${resp.data.token}` });
  });
  ...
  it("stores token, invalidates/refetches and navigates to home on success", async () => {
    ...
    const invalidateSpy = vi.spyOn(QueryClient.prototype as any, "invalidateQueries");
  ...
  it("shows error toast when request throws", async () => {
    ((userApi.loginRequest as unknown) as Mock).mockRejectedValue(new Error("fail"));
