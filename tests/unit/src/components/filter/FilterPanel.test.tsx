...
const calendarMock = (calendarModule as unknown as { __mock: CalendarMockApi })
  .__mock;
const toggleMock = (toggleGroupModule as unknown as { __mock: ToggleMockApi })
  .__mock;
...
const getFiltersForKey = (key: string) =>
  (useFilterStore.getState().filterStates[key]?.filters ??
    {}) as Partial<TestFilters>;
...
describe("FilterPanel", () => {
  ...
  it("rejects an end date before the selected start date", async () => {
    ...
    const departurePlaceholder = i18n.t(
      "reserveFilter.departureDatePlaceholder"
    );
  ...
  it("applies translation overrides when provided", () => {
    ...
    expect(
      screen.getByPlaceholderText(overriddenPlaceholder)
    ).toBeInTheDocument();
  });
  ...
  it("handles non-string search value gracefully", () => {
    ...
    renderFilter({
      filtersKey,
      initialFilters: { search: 123 as unknown as string },
    });
