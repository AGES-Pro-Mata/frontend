...
const setMockUseAdminRequests = (
  overrides?: Partial<UseAdminRequestsReturn>
) => {
  vi.mocked(mockUseAdminRequests).mockReturnValue({
    ...
    reservationStatus: overrides?.reservationStatus ?? [
      "Approved",
      "Pending",
      "Confirmed",
    ],
...
describe("AdminRequests Component", () => {
  ...
  it("calls approveMutation when approving a reservation request", () => {
    ...
    render(<AdminRequests />);
    fireEvent.click(
      screen.getByText((t) => t.includes("Reservation Requests"))
    );
    const approveButtons = screen.getAllByText("Approve");
  ...
  describe("requestsQuery.data fallback handling", () => {
    it("handles requestsQuery.data.data array", async () => {
      setMockUseAdminRequests({
        requestsQuery: {
          ...
          error: null,
          data: [
            { id: "9", name: "User9", email: "a@a.com", status: "Approved" },
          ],
        },
      });
      render(<AdminRequests />);
      expect(
        await screen.findByText((t) => t.includes("User9"))
      ).toBeInTheDocument();
    });
    ...
    it("handles requestsQuery.data.results array", async () => {
      ...
      render(<AdminRequests />);
      expect(
        await screen.findByText((t) => t.includes("User10"))
      ).toBeInTheDocument();
    });
  });
