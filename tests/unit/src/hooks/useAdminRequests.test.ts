...
describe("useAdminRequests", () => {
  ...
  it("fetches requests and approves an id (invalidate called)", async () => {
    ...
    (coreApi.api.post as unknown as Mock).mockResolvedValue({
      data: { ok: true },
    });

    const { result, queryClient } = renderHookWithProviders(() =>
      useAdminRequests({ page: 1 })
    );

    await waitFor(() =>
      expect(result.current.requestsQuery.data).toEqual(resp.data)
    );
    ...
    const postMock = coreApi.api["post"] as unknown as Mock;
  ...
  it("passes status param when provided and fetches accordingly", async () => {
    ...
    const { result } = renderHookWithProviders(() =>
      useAdminRequests({ page: 1, status: "APPROVED" })
    );
    ...
    const getMock = coreApi.api["get"] as unknown as Mock;

    expect(getMock).toHaveBeenCalledWith("/api/requests", {
