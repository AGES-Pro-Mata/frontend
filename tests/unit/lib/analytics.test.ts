...
describe("injectUmamiAnalytics", () => {
  ...
  it("does not inject script when disabled or missing config", () => {
    ...
    // disabled explicitly
    injectUmamiAnalytics({ enabled: false, scriptUrl: "http://s", websiteId: "w" });
  ...
  injectUmamiAnalytics({ enabled: true, scriptUrl: "http://s", websiteId: "" });
    expect(appended).toHaveLength(0);
  });
  ...
  it("does not inject if existing script is present", () => {
    ...
    g.document = {
      querySelector: () => ({ /* existing script */ }),
      createElement: () => ({ defer: false, src: "", dataset: {} }),
      head: { appendChild: (n: unknown) => appended.push(n) },
    } as unknown as Document & { head: { appendChild: (n: unknown) => void } };

    injectUmamiAnalytics({ enabled: true, scriptUrl: "http://s", websiteId: "w" });
    expect(appended).toHaveLength(0);
  });
  ...
  it("injects a script element when enabled and not present", () => {
    ...
    let created: { tag?: string; defer?: boolean; src?: string; dataset?: Record<string, string> } | null = null;
    ...
    injectUmamiAnalytics({ enabled: true, scriptUrl: "https://example.com/umami.js", websiteId: "site-123" });
    ...
    expect((created!.dataset as Record<string, string>)["websiteId"]).toBe("site-123");
    expect((created!.dataset as Record<string, string>)["analytics"]).toBe("umami");
  });
  ...
  G.process.env.VITE_ENABLE_ANALYTICS = "true";
    ...
    const { injectUmamiAnalytics: inject } = await import("../../../src/lib/analytics");

    const appended: unknown[] = [];

    g.document = {
      querySelector: () => null,
      createElement: (tag: string) => ({ tag, defer: false, src: "", dataset: {} }),
      head: { appendChild: (n: unknown) => appended.push(n) },
    } as unknown as Document & { head: { appendChild: (n: unknown) => void } };
  ...
  const G2 = globalThis as unknown as { process?: { env?: Record<string, string> } };
