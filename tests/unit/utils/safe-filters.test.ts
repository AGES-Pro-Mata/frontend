...
describe('safeParseFilters', () => {
  ...
  it('returns empty string for undefined filters', () => {
    const res = safeParseFilters(undefined as unknown as Record<string, unknown>, schema)
  ...
  it('propagates schema parse errors', () => {
    ...
    expect(() => safeParseFilters({ a: 'ok', b: 'not-a-number' as unknown as number }, schema)).toThrow()
  })
})
